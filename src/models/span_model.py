...
from src.models.query_head import SpanQueryDecoder
...
class VaaniQueryModel(nn.Module):
    """Same frontend and FPN as `VaaniSpanModel`; the head is swapped.

    TridentHead + SoftNMS + count head become `SpanQueryDecoder`, which
    predicts the final set of spans directly (see query_head.py for why). Only
    the base FPN level (`feats[0]`, the finest, 40 ms grid) feeds the decoder:
    events cannot exceed the 8 s window anyway, and queries do not need a
    dense per-level anchor grid the way TridentHead's point regression did, so
    the coarser pyramid levels buy nothing here that full self-attention over
    ~200 base-level tokens does not already give the decoder for free.

    Every auxiliary head (frame tags, class-agnostic Dice mask, speech, the
    20 ms boundary branch) is carried over unchanged from `VaaniSpanModel` -
    none of them read from TridentHead, all of them read `feats[0]` directly.
    The count head is dropped: the query decoder answers "how many" itself.
    """
    ...
    def __init__(self, n_class: int, n_frames: int, encoder: FusionEncoder | None = None,
                 ...
                 n_base_layers: int = 2, n_head: int = 8, dropout: float = 0.1,
                 use_specaug: bool = True, use_flux: bool = True,
                 boundary_branch: bool = True, boundary_mult: int = 2, hi_after: int = 1,
                 n_queries: int = 24, n_stages: int = 3,
                 coarse_delta: float = 4.0, refine_delta: float = 0.4):
        super().__init__()
        self.n_class, self.n_frames, self.fps = n_class, n_frames, float(fps)
        self.boundary_mult = int(boundary_mult)
        self.n_hi = int(n_frames * self.boundary_mult)
        ...
        self.fpn = TemporalFPN(d_in, d_model=d_model, n_levels=n_levels,
                               n_base_layers=n_base_layers, n_head=n_head,
                               dropout=dropout)
        self.head = SpanQueryDecoder(d_model=d_model, n_queries=n_queries, n_bins=n_bins,
                                     n_stages=n_stages, n_head=n_head, dropout=dropout,
                                     clip_len=n_frames / float(fps),
                                     coarse_delta=coarse_delta, refine_delta=refine_delta)
    ...
    def forward(self, wav: torch.Tensor, frame_valid: torch.Tensor | None = None) -> list:
        mel = self.logmel(wav, frame_valid)
        if self.use_flux:
            mel = torch.cat([mel, onset_strength(mel)], dim=1)
        mel = self.specaug(mel)
        h, hi = self.cnn(mel.contiguous(memory_format=torch.channels_last))
        ...
        outs = self.head(base, masks[0])

        frame_logits, clip_probs = self.frame_head(base, mask)
        agn_logits = self.agn_head(base).squeeze(-1)
        speech_logits = self.speech_head(base).squeeze(-1)
        aux = {"frame_logits": frame_logits, "clip_probs": clip_probs,
              "agn_logits": agn_logits, "speech_logits": speech_logits,
              "base_mask": mask}
        if self.boundary is not None:
            b = self.boundary(hi, base, self.n_hi)
            aux["onset_logits"] = b[:, 0]
            aux["offset_logits"] = b[:, 1]
            aux["hi_mask"] = resample_time(mask.unsqueeze(-1), self.n_hi).squeeze(-1)
        for o in outs:
            o.update(aux)
        return outs


def build_query_model(cfg: dict, n_class: int, encoder: FusionEncoder | None = None
                      ) -> VaaniQueryModel:
    d, m = cfg["data"], cfg["model"]
    n_frames = int(round(float(d["clip_len"]) * float(d["fps"])))
    q = m.get("query", {})
    return VaaniQueryModel(
        n_class=n_class, n_frames=n_frames, encoder=encoder,
        n_mels=int(d["n_mels"]), sr=int(d["sr"]), hop=int(d["hop"]), fps=float(d["fps"]),
        d_model=int(m.get("d_model", 384)), n_levels=int(m.get("n_levels", 5)),
        n_bins=int(m.get("n_bins", 16)), n_base_layers=int(m.get("n_base_layers", 2)),
        n_head=int(m.get("n_head", 8)), dropout=float(m.get("dropout", 0.1)),
        use_specaug=bool(m.get("specaug", True)), use_flux=bool(m.get("flux", True)),
        boundary_branch=bool(m.get("boundary_branch", True)),
        boundary_mult=int(m.get("boundary_mult", 2)), hi_after=int(m.get("boundary_tap", 1)),
        n_queries=int(q.get("n_queries", 24)), n_stages=int(q.get("n_stages", 3)),
        coarse_delta=float(q.get("coarse_delta", 4.0)),
        refine_delta=float(q.get("refine_delta", 0.4)))
