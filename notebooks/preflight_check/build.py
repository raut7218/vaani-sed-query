...
CELLS = [
    ...
    cell('import shutil, sys, time, torch\n'
         ...
         '    # truncate() makes a *sparse* file on ext/overlay filesystems - it\n'
         '    # extends the logical size without consuming real blocks, so the\n'
         '    # disk never actually gets close to full. posix_fallocate forces\n'
         '    # real allocation, which is the whole point of this test.\n'
         '    os.posix_fallocate(f.fileno(), 0, int(filler_gb * 1024**3))\n'
         '\n'
